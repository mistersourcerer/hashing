...
module Hashing
  ...
  module Hasherizer
    class Deprecation
      ...
      def deprecation_message(caller_first_line)
        called = caller_first_line.gsub /:in(.*)/, ""
        ["NOTE: `.hasherize' option :#{@method} is deprecated; ",
          "use :#{@replacement} instead. ",
          "It will be removed on or after version #{@version} ",
          "`.hasherize ..., :#{@method}' called from #{called}"].join
      end
    end
    ...
    class Options
      ...
      def alert_for_deprecations(caller_first_line)
        DEPRECATIONS.keys.each do |deprecated|
          if @options.has_key? deprecated
            warn DEPRECATIONS[deprecated].deprecation_message(caller_first_line)
          end
        end
      end
    end
    ...
    def hasherize(*ivars_and_options)
      ...
      @_options = Options.new raw_options
      @_options.filter caller.first
